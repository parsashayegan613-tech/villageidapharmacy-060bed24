...
serve(async (req) => {
  ...
  try {
    ...
    const { name, phone, currentPharmacy, medications, deliveryType } = payload;
    ...
    const html = `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
        <h2 style="color: #0f172a;">New Transfer Request 💊</h2>
        <div style="background-color: #f8fafc; padding: 20px; border-radius: 8px; margin-bottom: 24px;">
          <p><strong>Patient:</strong> ${name}</p>
          <p><strong>Phone:</strong> ${phone}</p>
          <p><strong>From Pharmacy:</strong> ${currentPharmacy}</p>
          <p><strong>Type:</strong> <span style="text-transform: capitalize;">${deliveryType || 'pickup'}</span></p>
          <p><strong>Medications:</strong> ${medications ? medications.map((rx: string) => `<br/>• ${rx}`).join("") : 'Not specified/See notes'}</p>
        </div>
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `New Transfer Request - ${name}`,
        html: html,
      }),
    });
