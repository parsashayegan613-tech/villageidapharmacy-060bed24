...
import { Search, ArrowRightLeft, User, Phone, Building2, FileText } from "lucide-react";
...
export default function AdminTransfers() {
    ...
    const saveAdminNote = async (id: string) => {
        ...
        const { error } = await supabase.from("transfers").update({ admin_notes: adminNote }).eq("id", id);
        setIsSavingNote(false);
        if (error) { toast.error("Failed to save note."); return; }
        toast.success("Admin note saved");
        setTransfers(prev => prev.map(t => t.id === id ? { ...t, admin_notes: adminNote } : t));
    ...
    const fetchTransfers = async () => {
        ...
        if (!error && data) { setTransfers(data); setFiltered(data); }
        setIsLoading(false);
    };
    ...
    useEffect(() => {
        ...
        if (search) result = result.filter(t => t.full_name.toLowerCase().includes(search.toLowerCase()) || t.current_pharmacy.toLowerCase().includes(search.toLowerCase()));
        ...
        setFiltered(result);
    }, [search, filterStatus, sortBy, transfers]);
    ...
    return (
        <AdminLayout>
            <div className="mb-6">
                <h1 className="text-2xl font-serif font-bold text-foreground">Transfers</h1>
                <p className="text-muted-foreground mt-1">Manage prescription transfer requests.</p>
            </div>

            <div className="flex flex-col sm:flex-row gap-3 mb-6">
                <div className="relative flex-1">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                    <Input placeholder="Search by name or pharmacy..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9" />
                </div>
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-muted/50 border-b border-border/60">
                                <tr>
                                    ...
                                    <th className="text-left px-5 py-3 font-semibold text-muted-foreground hidden md:table-cell">Old Pharmacy</th>
                            ...
                            <tbody className="divide-y divide-border/60">
                                {filtered.map(t => (
                                    <tr key={t.id} onClick={() => setSelected(t)} className="hover:bg-muted/30 cursor-pointer transition-colors">
                                        <td className="px-5 py-3.5">
                                            <p className="font-medium text-foreground">{t.full_name}</p>
                                            <p className="text-xs text-muted-foreground">{t.phone}</p>
                                        </td>
                                        <td className="px-5 py-3.5 hidden md:table-cell text-muted-foreground">{t.current_pharmacy}</td>
                                        <td className="px-5 py-3.5"><StatusBadge status={t.status} /></td>
                                        <td className="px-5 py-3.5 hidden sm:table-cell text-muted-foreground text-xs">{format(new Date(t.created_at), "MMM d, h:mm a")}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
            ...
            <Sheet open={!!selected} onOpenChange={() => setSelected(null)}>
                <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
                    {selected && (
                        <>
                            <SheetHeader><SheetTitle className="text-xl font-serif">Transfer Details</SheetTitle></SheetHeader>
