...
const timeSlots = [
    ...
    "3:00 PM", "3:15 PM", "3:30 PM", "3:45 PM", "4:00 PM", "4:15 PM", "4:30 PM", "4:45 PM", "5:00 PM",
];
...
function AppointmentsContent() {
    ...
    const getAvailableTimes = () => {
        ...
        if (isToday(date)) {
            const now = new Date();
            return timeSlots.filter((time) => { try { return parse(time, "h:mm a", date) > now; } catch { return true; } });
        }
        return timeSlots;
    };
                ...
                <div className="max-w-2xl mx-auto">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-10">
                        {appointmentTypes.map((type) => (
                            <button key={type.id} onClick={() => setSelectedType(type.id)} className={cn("p-4 rounded-2xl border-2 text-left transition-all", selectedType === type.id ? "border-primary bg-primary/5" : "border-border hover:border-muted-foreground/30")}>
                    ...
                    {selectedType && (
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                ...
                                <RadioGroup value={formData.contactMethod} onValueChange={(value) => setFormData(prev => ({ ...prev, contactMethod: value }))} className="mt-2 flex gap-4">
                                    <div className="flex items-center space-x-2"><RadioGroupItem value="call" id="call" /><Label htmlFor="call" className="font-normal">Call</Label></div>
                                    <div className="flex items-center space-x-2"><RadioGroupItem value="text" id="text" /><Label htmlFor="text" className="font-normal">Text</Label></div>
                                </RadioGroup>
                            </div>
                                ...
                                <Popover>
                                    <PopoverTrigger asChild>
                                        <Button variant="outline" className={cn("w-full justify-start text-left font-normal h-11", !date && "text-muted-foreground")}>
                                            ...
                                            {date && formData.preferredTime ? `${format(date, "PPP")} at ${formData.preferredTime}` : date ? `${format(date, "PPP")} (Select a time)` : <span>Pick a date & time</span>}
                                        </Button>
                                    </PopoverTrigger>
                                    <PopoverContent className="w-auto p-0 flex flex-col sm:flex-row" align="start">
                                        <div className="p-2">
                                            <Calendar mode="single" selected={date} onSelect={setDate} disabled={(d) => d < new Date(new Date().setHours(0, 0, 0, 0))} initialFocus className="border-none" />
                                        </div>
                                        {date && (
                                                ...
                                                {getAvailableTimes().length > 0 ? (
                                                    <div className="grid grid-cols-2 sm:grid-cols-1 gap-2">
                                                        {getAvailableTimes().map((time) => (
                                                            <Button key={time} type="button" variant={formData.preferredTime === time ? "default" : "outline"} className="w-full h-9 text-xs" onClick={() => setFormData(prev => ({ ...prev, preferredTime: time }))}>{time}</Button>
                                                        ))}
                                                    </div>
                                                ) : (
                                                    <div className="text-sm text-muted-foreground text-center py-8">No more times available today.</div>
                                                )}
                                            </div>
                                        )}
                                    </PopoverContent>
...
export function AppointmentsForm() {
    return (
        <Suspense fallback={<div className="min-h-[400px] flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary" /></div>}>
            <AppointmentsContent />
        </Suspense>
    );
}
